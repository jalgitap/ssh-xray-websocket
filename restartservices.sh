...
banner
echo -e "${kubali}Restarting Nginx${mwisho}"
sleep 1
systemctl restart nginx

echo -e "${kubali}Restarting Dropbear${mwisho}"
sleep 1
systemctl restart dropbear

echo -e "${kubali}Restarting websocket${mwisho}"
sleep 1
systemctl restart websocket

echo -e "${kubali}Restarting cron${mwisho}"
sleep 1
systemctl restart nginx

echo -e "${kubali}Restarting Vmess TLS${mwisho}"
sleep 1
systemctl restart vmtls

echo -e "${kubali}Restarting Vless TLS${mwisho}"
sleep 1
systemctl restart vltls

echo -e "${kubali}Restarting Trojan TLS${mwisho}"
sleep 1
systemctl restart trtls

echo -e "${kubali}Restarting Vmess NON TLS${mwisho}"
sleep 1
systemctl restart vmntls

echo -e "${kubali}Restarting Vless NON TLS${mwisho}"
sleep 1
systemctl restart vlntls

echo -e "${kubali}Restarting Trojan NON TLS${mwisho}"
sleep 1
systemctl restart trntls

if systemctl is-active --quiet  nginx; then
    nginxstatus="${kubali}\033[1mDone✅${mwisho}"
else
    nginxstatus="${kataa}\033[1mFailed❌${mwisho}"
fi        

if systemctl is-active --quiet  websocket; then
    websocketstatus="${kubali}\033[1mDone✅${mwisho}"
else
    websocketstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet  dropbear; then
    dropbearstatus="${kubali}\033[1mDone✅${mwisho}"
else
    dropbearstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet  vmtls; then
    vmtlsstatus="${kubali}\033[1mDone✅${mwisho}"
else
    vmtlsstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet  vmntls; then
    vmntlsstatus="${kubali}\033[1mDone✅${mwisho}"
else
    vmntlsstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet  vltls; then
    vltlsstatus="${kubali}\033[1mDone✅${mwisho}"
else
    vltlsstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet  vlntls; then
    vlntlsstatus="${kubali}\033[1mDone✅${mwisho}"
else
    vlntlsstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet  trtls; then
    trtlsstatus="${kubali}\033[1mDone✅${mwisho}"
else
    trtlsstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet  trntls; then
    trntlsstatus="${kubali}\033[1mDone✅${mwisho}"
else
    trntlsstatus="${kataa}\033[1mFailed❌${mwisho}"
fi

if systemctl is-active --quiet cron; then
    cronstatus="${kubali}\033[1mDone✅${mwisho}"
else
    cronstatus="${kataa}\033[1mFailed❌${mwisho}"
fi
sleep 1
clear
